...
int quickroot(string filebase="", int njob=0, int dotow = 0)
{
  ...
  */
  ...
  TH2D* h2_cluster_eta_phi = new TH2D("h2_cluster_eta_phi","",24,-1.1,1.1,64,-M_PI,M_PI);
  ...
  for(int h=nosim; h<2; ++h)
      ...
      for(int i=0; i<4; ++i)
	{
	  if(i<3)
	    {
	      h1_calo_occ[i] = new TH1D(("h1_calo_occ"+to_string(i)).c_str(),"",300,0,0.3);
	      ...
	      h2_occ_E[i] = new TH2D(("h2_occ_E"+to_string(i)).c_str(),"",300,0,0.3,100,0,(i==1?10:50));
	    }
  ...
  for(int h=nosim; h<2; ++h)
    {
      for(int i=0; i<4; ++i)
	{
	  h1_dphi[h][i] = new TH1D(("h1_dphi"+to_string(h)+"_"+to_string(i)).c_str(),"",32,0,M_PI);
	}
    }
  ...
  TH2D* event_disrt[4];
  TH2D* event_sum = new TH2D("event_sum","Calorimeter Sum",24,-1.1,1.1,64,0,2*M_PI);
  TH1D* jetfrac[2][4];
  
  TH1D* jetE[2][4];
  
  TH1D* ljetE = new TH1D("ljetE","",600,0,60);
  TH1D* ljetEta = new TH1D("ljetEta","",20,-1.1,1.1);
  TH1D* tjetE = new TH1D("tjetE","",600,0,60);
  
  TH1D* hejet[2];
  TH1D* trigturn[4];
  for(int i=0; i<4; ++i)
    {
      trigturn[i] = new TH1D(("trigturn_"+to_string(i)).c_str(),"",26,4,30);
    }
  cout << "test1" << endl;
  for(int h=nosim; h<2; ++h)
    {
      ...
      for(int i=0; i<4; ++i)
	{
	  ...
	  if(i==1) calo = "IHCal";
	  if(i==2) calo = "OHCal";
	  jetE[h][i] = new TH1D(("jetE"+to_string(h)+"_"+to_string(i)).c_str(),"",700,0,70);
	  
	  jetfrac[h][i] = new TH1D(("jetfrac"+to_string(h)+"_"+to_string(i)).c_str(),"",100,0,1);
	  event_disrt[i] = new TH2D(("event_display_rt"+to_string(h)+"_"+to_string(i)).c_str(),calo.c_str(),24,-1.1,1.1,64,0,2*M_PI);
	  /*
  ...
  */
  ...
  //int cluster_n;
  cout << "begin getting branches" << endl;
  /*
  if(dotow)
    {
  tree[1]->SetBranchAddress("sector_rtem",&sector_rtem);
  tree[1]->SetBranchAddress("rtemen",e_rtem);
  tree[1]->SetBranchAddress("rtemet",eta_rtem);
  tree[1]->SetBranchAddress("rtemph",phi_rtem);
  ...
  */
  ...
  for(int h=nosim; h<2; ++h)
    {
      //if(dotow) tree[h]->SetBranchAddress("mbenrgy",mbdq);
      ...
      if(dotow)
	{
      ...
      tree[h]->SetBranchAddress("sectorem",&sectorrt[h][0]);
      tree[h]->SetBranchAddress("emcalen",enrt[h][0]);
      tree[h]->SetBranchAddress("emcaletabin",etart[h][0]);
      tree[h]->SetBranchAddress("emcalphibin",phirt[h][0]);
	}
      */
      tree[h]->SetBranchAddress("jet_e",jet_e[h]);
      tree[h]->SetBranchAddress("jet_et",jet_et[h]);
      tree[h]->SetBranchAddress("jet_ph",jet_ph[h]);
      tree[h]->SetBranchAddress("failscut",&failscut);
      /*
      if(dotow)
	{
  ...
  cout << "begin filling alcetahists" << endl;
  /*
  for(int i=0; i<jett[1]->GetEntries(); ++i)
    {
      //cout << i << "/" << jett[1]->GetEntries() << endl;
      jett[1]->GetEntry(i);
  ...
  */
  /*
  cout << "end filling acelthists" << endl;
  jetfile->WriteObject(acetahist,acetahist->GetName());
  jetfile->WriteObject(alcethist,alcethist->GetName());
  cout << "wrote jetfile alcethist" << endl;
  int cancount = 0;
  TCanvas* c = new TCanvas("","",1100,600);
  ...
  for(int i=0; i<3; ++i)
    {
      ...
      event_disrt[i]->GetXaxis()->SetNdivisions(2,kFALSE);
  ...
  event_sum->GetXaxis()->SetNdivisions(2,kFALSE);
  ...
  */
  ...
  for(int i=0; i<4; i++)
    {
      //for(int j=0; j<4; ++j)
      //{
      jetTrigE[i] = new TH1D(("jetTrigE"+to_string(i)).c_str(),"",700,0,70);
      jet8trig[i] = new TH1D(("jet8trig"+to_string(i)).c_str(),"",700,0,70);
  ...
  for(int i=0; i<tree[h]->GetEntries(); ++i)
      ...
      for(int k=0; k<njet[h]; ++k)
	{
      ...
      if((trigvec >> 10) & 1)
	{
	  trigturn[0]->Fill(maxjet[0]);
	}
      if((trigvec>>17) &1)
	{
	  trigturn[1]->Fill(maxjet[0]);
	}
      if((trigvec>>18) &1)
	{
	  trigturn[2]->Fill(maxjet[0]);
	}
      if((trigvec>>19) &1)
	{
	  trigturn[3]->Fill(maxjet[0]);
	}
	  ...
	  for(int k=0; k<njet[h]; ++k)
	      ...
	      for(int l=0; l<ncircle; ++l)
		{
	      ...
	      float phitext = jet_ph[h][k]+M_PI+(jet_ph[h][k]+M_PI>5?-0.57:0.45);
	      phivals[k] = phitext;
	      if(k>0)
		{
	      ...
	      drawText((e_string+" GeV").c_str(),(jet_et[h][k]>0?1.0:-1.0),phitext,(jet_et[h][k]>0?1:0),kBlack,0.08,42,false);
	    }
	  c->cd(0);
      ...
      */
      ...
      if(!(trigvec >> 17 & 1 || ismb[h]))//!ismb[h])// && !trigvecproxy && filename != simliststr)
      {
	  ...
	  for(int j=0; j<ntj; ++j)
	    {
	      ...
	      for(int k=0; k<ntj; ++k)
		{
		  if(abs(tjet_eta[j]) > 1.1 || abs(tjet_eta[k]) > 1.1) continue;
		  if(j==k) continue;
		  float ET2 = tjet_e[k]/cosh(tjet_eta[k]);
		  if(ET < ET2) continue;
		  float AJ = (ET-ET2)/(ET+ET2);
		  if(AJ < 0) cout << ET << " " << ET2 << " " << AJ << endl;
		  float odph = abs(tjet_phi[j]-tjet_phi[k]);
		  if(odph < 0) odph += 2*M_PI;
		  if(odph > M_PI) odph -= 2*(odph-M_PI);
		  if(odph > 7*M_PI/8)
		    {
		      h1_tAJ[0]->Fill(AJ);
		      if(ET > 20 && ET2 > 10) h1_tAJ[2]->Fill(AJ);
		      if(ET > 15 && ET2 > 7) h1_tAJ[1]->Fill(AJ);
		      if(ET > 25 && ET2 > 15) h1_tAJ[3]->Fill(AJ);
		      
		      if(ET > 20 && ET2 > 8) h1_tAJ[8]->Fill(AJ);
		      if(ET > 20 && ET2 > 10) h1_tAJ[10]->Fill(AJ);
		      if(ET > 20 && ET2 > 12) h1_tAJ[9]->Fill(AJ);
		      if(ET > 30 && ET2 > 15) h1_tAJ[11]->Fill(AJ);
		      
		    }
		  if(odph > 3*M_PI/4)
		    {
		      if(ET > 20 && ET2 > 10) h1_tAJ[4]->Fill(AJ);
		      if(ET > 15 && ET2 > 7) h1_tAJ[5]->Fill(AJ);
		      if(ET > 25 && ET2 > 15) h1_tAJ[6]->Fill(AJ);
		      if(ET > 30 && ET2 > 12) h1_tAJ[7]->Fill(AJ);
		    }
      ...
      for(int k=0; k<njet[h]; ++k)
	{
	  ...
	  */
	  ...
	  //if(dotow) hejet[h]->Fill(ehjet[h][k]);
	  if(ismb[h])
	    {
	      jetE[h][0]->Fill(ET);
	      if(!(failscut & 0x1)) jetE[h][1]->Fill(ET);
	      if(!(failscut & 0x2)) jetE[h][2]->Fill(ET);
	      if(!(failscut & 0x1) && !(failscut & 0x2)) jetE[h][3]->Fill(ET);
	    }
	  if(trigvec >> 17 & 1)
	    {
	      jet8trig[0]->Fill(ET);
	      if(!(failscut & 0x1))
		{
	      ...
	      if(!(failscut & 0x2)) jet8trig[2]->Fill(ET);
	      if(!(failscut & 0x1) && !(failscut & 0x2)) jet8trig[3]->Fill(ET);
	    }
	  int whichjet = 0;
	  if(ET < 6) whichjet = 0;
  ...
  for(int i=0; i<3; ++i)
    {
      ...
      outfile->WriteObject(tjet_dphi[i],tjet_dphi[i]->GetName());
    }
	  ...
	  if(i < 4)
	    {
	      ...
	      if(h==1) outfile->WriteObject(jet8trig[i],jet8trig[i]->GetName());
	      for(int j=0; j<5; ++j)
		{
