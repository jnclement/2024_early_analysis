...
echo "output = /sphenix/tg/tg01/jets/jocl/out/${RN}/output_${BASENAME}_\$(Process).out" >> $SUBNAME
echo "should_transfer_files   = IF_NEEDED" >> $SUBNAME
echo "when_to_transfer_output = ON_EXIT" >> $SUBNAME
echo "error = /sphenix/tg/tg01/jets/jocl/err/${RN}/error_${BASENAME}_\$(Process).err" >> $SUBNAME
