...
if [ $1 -eq 1 ]; then
	...
	echo "output = /sphenix/user/jocl/projects/run2024_earlydata/run/output/out/output_${BASENAME}.out" >> $SUBNAME
	echo "should_transfer_files   = IF_NEEDED" >> $SUBNAME
	echo "when_to_transfer_output = ON_EXIT" >> $SUBNAME
	echo "error = /sphenix/user/jocl/projects/run2024_earlydata/run/output/err/error_${BASENAME}.err" >> $SUBNAME
