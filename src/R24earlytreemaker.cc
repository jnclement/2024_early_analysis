...
int R24earlytreemaker::process_event(PHCompositeNode *topNode)
{
  ...
  */
  ...
  for(int i=0; i<ntj; ++i)
    {
      if(tjet_pt[i] > max_tjet_et) max_tjet_et = tjet_pt[i];
    }
  ...
  */
  ...
  float zvtx = vtx[2];
  if(_debug > 1) cout << "got zvtx" << endl;
  ...
  */
      ...
      for(int i=0; i<tocheck; ++i)
	{
	  ...
	  for(auto comp: jet->get_comp_vec())
	      ...
	      if(comp.first == 13 || comp.first == 28 || comp.first == 25)
		{
		  ...
		  float radius = 93.5;//tower_geom->get_center_radius();//93.5;
		  float ihEta = tower_geom->get_eta();
		  float emZ = radius/(tan(2*atan(exp(-ihEta))));
  ...
  if(!njet && !_dotow && !ntj) return Fun4AllReturnCodes::EVENT_OK;
